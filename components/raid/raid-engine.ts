...
/** 获取第一个可用的 API 配置，没有则抛错。 */
function getApiConfig(): ApiConfig {
    const configs = loadApiConfigs();
    const config = configs[0];
    if (!config) throw new Error("未配置 API，请先在设置中添加 API 配置。");
    return config;
}
...
export async function generateStoryBeat(params: GenerateStoryBeatParams): Promise<StoryBeat> {
    const { dungeon, choiceText } = params;
    const config = getApiConfig();
    ...
    const systemPrompt = [
        ...
        `2. choices 生成 2-4 个选项，每个含 text（选项内容）、hint（后果提示，如"勇气+5"或"风险：可能激怒对方"）、riskLevel。风险分布：${riskGuide}`,
        ...
        "  ],",
        '  "choices": [',
        '    { "text": "选项", "hint": "后果提示", "riskLevel": "safe" | "risky" | "deadly" }',
        "  ],",
    ...
    return buildStoryBeat(parsed, dungeon, isDeathTriggered, favorMod);
}
    ...
    if (!isDeath && !isClimax) {
        ...
        if (choices.length < 2) {
            const fallbacks: StoryChoice[] = [
                { id: `${beatId}_safe`, text: "谨慎应对", hint: "稳妥推进", riskLevel: "safe" },
                { id: `${beatId}_risky`, text: "主动出击", hint: "可能改变局势", riskLevel: "risky" },
            ];
            let fi = 0;
