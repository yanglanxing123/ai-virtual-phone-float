...
const DEFAULT_FISH_SPEED = 0.9;
const FISH_SPEED_MIN = 0.5;
const FISH_SPEED_MAX = 2.0;
const FISH_SPEED_STEP = 0.1;
const DEFAULT_FISH_MODEL = "s2.1-pro-free";

const FISH_MODEL_OPTIONS = [
    { id: "s2.1-pro-free", name: "s2.1-pro-free（免费推荐）" },
...
const VOICE_PROVIDER_OPTIONS = [
    ...
    { value: "FishAudio", label: "鱼声 Fish Audio" },
];
        ...
        .map(config => {
            if (config.provider === "FishAudio") {
                const speechSpeed = typeof config.speechSpeed === "number" && Number.isFinite(config.speechSpeed)
                    ? Math.min(FISH_SPEED_MAX, Math.max(FISH_SPEED_MIN, config.speechSpeed))
                    : DEFAULT_FISH_SPEED;
                return { ...config, speechSpeed };
            }
    ...
    const updateProvider = (id: string, providerOption: string) => {
        ...
        if (providerOption === "FishAudio") {
            ...
            updateConfig(id, {
                provider: "FishAudio",
                baseUrl: undefined,
                model: wasFish ? (current?.model || DEFAULT_FISH_MODEL) : DEFAULT_FISH_MODEL,
                defaultVoice: wasFish ? (current?.defaultVoice || "") : "",
                speechSpeed: wasFish ? (current?.speechSpeed ?? DEFAULT_FISH_SPEED) : DEFAULT_FISH_SPEED,
            });
    ...
    const fetchVoices = async (config: VoiceApiConfig) => {
        ...
        try {
            if (config.provider === "Minimax") {
                ...
                setFetchedVoices(prev => ({ ...prev, [config.id]: config.customVoices || [] }));
                setFetchError(prev => ({ ...prev, [config.id]: "鱼声通过 reference_id 选音色，在 fish.audio 网页复制后直接填入" }));
            } else {
                                ...
                                return (
                                    <div className="flex flex-col gap-4">
                                        ...
                                        {config.provider === "FishAudio" && (
                                            <>
                                                <div className="flex flex-col gap-1">
                                                    <label className="menu-desc ml-1">语音模型 (Fish Model)</label>
                                                    {manualModelIds[config.id] ? (
                                                        <div className="flex gap-2">
                                                            <Input
                                                                type="text"
                                                        ...
                                                        <select
                                                            value={FISH_MODEL_OPTIONS.some(m => m.id === config.model) ? config.model : "__manual__"}
                                                            onChange={(e) => {
                                                                if (e.target.value === "__manual__") {
                                                                    setManualModelIds(prev => ({ ...prev, [config.id]: true }));
                                                                    return;
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center justify-between px-1">
                                                        <label className="menu-desc">语速</label>
                                                        <span className="menu-label font-medium">{(config.speechSpeed ?? DEFAULT_FISH_SPEED).toFixed(1)}×</span>
                                                    </div>
                                                    <input
                                                        type="range"
                                                        min={FISH_SPEED_MIN}
                                                        max={FISH_SPEED_MAX}
                                                        step={FISH_SPEED_STEP}
                                                        value={config.speechSpeed ?? DEFAULT_FISH_SPEED}
                                                        onChange={(e) => updateConfig(config.id, { speechSpeed: Number(e.target.value) })}
                                                        className="w-full accent-black"
                                                        aria-label="鱼声语速"
                                                    />
                                                    <div className="relative mt-1 h-5 px-1 text-xs text-gray-500" aria-hidden="true">
                                                        <span className="absolute left-1 whitespace-nowrap">{FISH_SPEED_MIN.toFixed(1)}×</span>
                                                        <span className="absolute whitespace-nowrap" style={{ left: "26.666%", transform: "translateX(-50%)" }}>0.9× 默认</span>
                                                        <span className="absolute right-1 whitespace-nowrap">{FISH_SPEED_MAX.toFixed(1)}×</span>
                                                    </div>
                                                </div>
                                            </>
                                        )}
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <label className="menu-desc ml-1">
                                                {config.provider === "FishAudio"
                                                    ? "音色 reference_id（粘贴 id 或 fish.audio 链接）"
                                                    : "默认音色 (Default Voice) 或 自定义 Voice ID"}
                                            </label>
                                            <div className="flex flex-col gap-2">
                                                <div className="flex gap-2">
                                                    {manualVoiceIds[config.id] ? (
                                                        <>
                                                            <Input
                                                                ...
                                                                placeholder={config.provider === "OpenAI" ? "alloy" : config.provider === "FishAudio" ? "粘贴 32 位 reference_id 或 fish.audio 链接" : "male-qn-qingse 或克隆 Voice ID"}
                                                                className="flex-1"
                                                            />
                                                            <button
                                                                type="button"
                                                ...
                                                {config.provider === "FishAudio" && (
                                                    <p className="text-[11px] text-slate-400 mt-1 pl-1">
                                                        在 <a href="https://fish.audio/zh-CN/developers/" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline font-semibold">fish.audio 开发者页</a> 拿 API Key（<span className="text-amber-600 font-medium">需梯子</span>）。音色在 fish.audio 语音库选好后，复制 reference_id 或整条链接贴到上方输入框，会自动识别。
                                                    </p>
                                                )}
