...
async function proxyAsset(url: string, requestHeaders?: unknown, timeoutMsValue?: unknown) {
  let target: URL;
  ...
  const timeoutMs = Math.max(3000, Math.min(20000, Number(timeoutMsValue) || 12000));
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const headers = cleanHeaders(requestHeaders);
...
export async function GET(request: NextRequest) {
  try {
    const url = request.nextUrl.searchParams.get("url") || request.nextUrl.searchParams.get("asset");
    if (!url) return NextResponse.json({ ok: false, error: "缺少 URL" }, { status: 400 });
    const referer = request.nextUrl.searchParams.get("referer");
    return proxyAsset(url, referer ? { Referer: referer } : undefined, request.nextUrl.searchParams.get("timeoutMs"));
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "资源代理失败" }, { status: 502 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url) return NextResponse.json({ ok: false, error: "缺少 URL" }, { status: 400 });
    if (body?.asset === true) return proxyAsset(url, body?.headers, body?.timeoutMs);
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "请求源站失败" }, { status: 502 });
  }
}
