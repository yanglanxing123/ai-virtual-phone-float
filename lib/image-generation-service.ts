import type { ImageGenerationSettings, ImageApiConfig } from "./settings-types";
import { loadImageGenerationSettings, getActiveImageApiConfig } from "./settings-storage";
...
function normalizeBaseUrl(baseUrl: string): string {
  let url = baseUrl
    ...
    .replace(/\/images$/i, "");
  // 如果 URL 没有以 /v1 结尾，自动补全（OpenAI 兼容 API 的标准路径）
  if (!/\/v\d+$/i.test(url)) {
    url += "/v1";
  }
  return url;
...
export async function fetchImageGenerationModels(settings: ImageGenerationSettings): Promise<string[]> {
    const config = getActiveImageApiConfig(settings);
    if (!config) return [];
    const { apiKey, baseUrl, requestMode } = config;
    if (requestMode === "direct") {
        try {
            ...
            // 防 HTML 响应：如果返回的不是 JSON 格式，给出友好提示
            const trimmed = text.trim();
            if (trimmed.startsWith("<") || trimmed.startsWith("<!")) {
                throw new Error("API 返回了 HTML 而非 JSON，请检查 Base URL 是否正确（需以 /v1 结尾，例如 https://api.example.com/v1）");
            }
            let parsed;
            try {
                parsed = JSON.parse(trimmed);
            } catch {
                throw new Error("API 返回的内容不是有效的 JSON，请检查 Base URL 是否包含 /v1 路径");
            }
            return extractModels(parsed);
        } catch (error) {
...
async function generateImageDirect(params: {
  ...
  const { config, prompt, referenceImageDataUrl, signal, proxyBaseUrl } = params;
  throwIfAborted(signal);
  const hasReference = Boolean(referenceImageDataUrl);
  const url = buildImageUrl(proxyBaseUrl || config.baseUrl, hasReference ? "edits" : "generations");
  const headers: Record<string, string> = { Authorization: `Bearer ${config.apiKey}` };
  if (proxyBaseUrl) headers["x-upstream-base-url"] = normalizeBaseUrl(config.baseUrl);
  let body: BodyInit;
...
async function generateImageViaServer(params: {
  ...
  const { config, prompt, referenceImageDataUrl, signal } = params;
  throwIfAborted(signal);
...
export async function generateImageFromConfiguredApi(params: {
  ...
  /** 直接传入参考图 dataUrl（优先于 characterId 参考图系统） */
  ...
  if (params.referenceImageDataUrl) {
    ...
    referenceImageDataUrl = await normalizeReferenceImageForEdit(params.referenceImageDataUrl);
  } else {
    ...
    const rawReferenceImageDataUrl = params.useReferenceImage && reference?.assetId
      ...
      : null;
    throwIfAborted(params.signal);
    referenceImageDataUrl = rawReferenceImageDataUrl
