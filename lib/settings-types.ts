...
// provider 可选值: "Minimax" | "OpenAI" | "FishAudio"
// - Minimax: 国内/海外 T2A, hex-encoded mp3, voice_id 选音色
// - OpenAI: /audio/speech, binary blob, 预设音色名
// - FishAudio: api.fish.audio/v1/tts, binary mp3, reference_id 选音色, model 头传模型
export type VoiceApiConfig = {
    ...
    /** Minimax voice_setting.speed / FishAudio prosody.speed. Missing → 1.0 (Minimax) / 0.9 (FishAudio). */
    speechSpeed?: number;
...
/** 单个生图 API 方案。与 VoiceApiConfig 同构，支持多方案并存、切换使用。 */
...
export type ImageGenerationSettings = {
    enabled: boolean;
    /** 多方案列表（类似语音 API 的 configs 数组）。 */
    configs: ImageApiConfig[];
    /** 当前激活的方案 id；为空时回退到 configs[0]。 */
...
export type ContentAppId =
    ...
    | "moments" | "group_chat" | "vn" | "adventure" | "raid";
