            ...
            <div className="reading-hub-drawer-backdrop" onClick={() => setSourceDrawerOpen(false)}>
              <aside className="reading-hub-drawer" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="reading-hub-drawer-actions">
                  <button type="button" onClick={() => { setSourceImportTarget(sourceDrawerMode); sourceFileInputRef.current?.click(); }}><Plus size={16} /> 新增书源</button>
                ...
                <div className="reading-hub-drawer-section">
                  ...
                  {drawerSources.map((source) => (
                    <div key={source.id} className="reading-hub-drawer-source">
                      ...
                      <button type="button" className="reading-hub-drawer-source-remove" title="删除书源" aria-label={`删除书源 ${source.name}`} onClick={() => {
                        if (!window.confirm(`确定要删除书源“${source.name}”吗？\n删除后需要重新导入才能恢复。`)) return;
                        removeReadingSource(source.id);
                        setBookSources(loadReadingSources());
                        persistSourceSet("home", homeSourceIds.filter((id) => id !== source.id));
                        persistSourceSet("discovery", discoverySourceIds.filter((id) => id !== source.id));
                        setHomeModules((prev) => {
                          const next = prev.filter((module) => module.sourceId !== source.id);
                          try { window.localStorage.setItem("reading-home-modules-v2", JSON.stringify(next)); } catch {}
                          return next;
                        });
                        setHomeModuleData((prev) => {
                          const next = { ...prev };
                          for (const key of Object.keys(next)) {
                            const module = homeModules.find((item) => item.id === key);
                            if (module?.sourceId === source.id) delete next[key];
                          }
                          return next;
                        });
                        if (selectedSourceId === source.id) {
                          const remaining = loadReadingSources();
                          setSelectedSourceId(remaining.find((item) => item.enabled)?.id || remaining[0]?.id || "");
                          setSourceResults([]); setGenericResults([]);
                          setSelectedSourceBook(null); setGenericBook(null);
                          setSourceDetail(null); setGenericDetail(null);
                          setSourceChapters([]); setGenericChapters([]);
                        }
                        setSourceMessage(`已删除书源：${source.name}`);
